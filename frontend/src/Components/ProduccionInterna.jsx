...
export const ProduccionInterna = () => {
    ...
    const guardarLote = async (e) => {
        ...
        try {
            ...
            const res = await ApiStockBloom.registrarProduccion(payload);
            ...
            if (res && res.success) {
                setModalConfig({
                    ...
                    mensaje: `El lote de cultivo para la planta [${nombrePlanta}] con ${form.cantidad} piezas ha sido agregado con éxito al sistema.`
                });

                // Limpieza de campos
                setForm({ id_planta: '', cantidad: '', fecha_siembra: '', fecha_cosecha: '' });
                setBusquedaPlanta('');
                cargarHistorial();
            } else {
                setModalConfig({
